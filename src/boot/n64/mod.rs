    ...
    loop {
        ...
        #[cfg(feature = "alloc")]
        {
            ...
            if let Some(msg) = _info.message() {
                vi.print_string(2, line, Colour::WHITE, &msg.to_string());
                line += 4;
            }
        }
...
unsafe extern "C" fn _setup() {
    ...
    setup_globals();
    if !is_bbplayer() {
        from_raw_parts_mut::<u32>(phys_to_k1_usize(0x1FC0_07FC) as *mut (), ()).write_volatile(8);
    }
    setup_ints();
}
